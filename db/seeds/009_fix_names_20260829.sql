...
-- 目的     : 名前の前後空白の除去、FamilyName の欠落補完、重複スタイルの整理
...
-- 1) 前後の空白を除去
UPDATE `style`    SET `StyleName`   = TRIM(`StyleName`)   WHERE `StyleID`    = 'st0002';
UPDATE `products` SET `ProductName` = TRIM(`ProductName`) WHERE `ProductID`  = 'pr0013';

-- 2) FamilyName の欠落を補完(style_group() の判定結果は変わらない)
UPDATE `style` SET `FamilyName` = 'IPA'             WHERE `StyleID` = 'st0002' AND (`FamilyName` IS NULL OR `FamilyName` = '');
UPDATE `style` SET `FamilyName` = 'Wild/Sour Beers' WHERE `StyleID` = 'st0001' AND (`FamilyName` IS NULL OR `FamilyName` = '');

-- 3) 重複した空のスタイルに注記を付け、誤用を防ぐ
UPDATE `style` SET `StyleName` = 'New England IPA (未使用・st0002と重複)'
  WHERE `StyleID` = 'st0019';
