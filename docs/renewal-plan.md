...
## DB適性の確認と文字化けバグの修正 (2026-08-26)

「説明文を持つのに今のDBで適切か」というユーザーの問いを受けて実測した。

**結論: MySQL(RDS)で適切**。移行の必要なし。
- 説明文の実測: ProductExplain 38件 / 平均128文字 / 最長212文字。TEXT型で十分
- データは products↔maker↔style の関係が主でRDB向き。説明文は多数ある列の一つに過ぎず、
  ドキュメントDBに切り出す利点がない(運用先が増えるだけ)
- 規模も38件。件数が数千〜万に増えても MySQL で問題ない

**ただし実害のあるバグを発見し修正した(重要)**
- DB・カラムは `utf8mb4 / utf8mb4_0900_ai_ci` だが、**サーバ既定が `utf8mb3`** で、
  アプリのDSNに charset 指定が無かった → **接続が utf8mb3 になり、絵文字やサロゲートペア漢字
  (🍺 𠮟 𩸽 等)がエラーも警告もなく `????` に化けていた**
- 実測: 修正前は「絵文字🍺🌌」→「絵文字????????」。修正後はOK
- 対処: `db_config.local.php` の DSN に `;charset=utf8mb4` を追加(リポジトリの .example、
  ローカル、サーバの dev/prod すべて)。既存38件に破損は無かった(?を含む行0件)
- **紹介文を書き込むパイプラインを作る前に見つけられたのが幸い**。稼働後だと静かにデータが壊れ続けていた

**将来の検討事項(いまは不要)**
- 全文検索: 現在は38件をJSでクライアント絞り込み。件数が増えたら MySQL 8 の FULLTEXT +
  ngram パーサ(`ngram_token_size=2` は設定済み)を検討する
